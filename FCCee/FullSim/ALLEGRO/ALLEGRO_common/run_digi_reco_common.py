# run_digi_reco_common.py
# Common Gaudi steering utilities shared across ALLEGRO detector configurations.
#
# Provides:
#   - str2bool              : CLI argument helper
#   - add_parser_args       : register all common CLI options onto a parser
#   - setupSWClusters       : configure sliding-window clustering sequence
#   - setupTopoClusters     : configure topological clustering sequence
#   - run_digi_reco         : full digitization/reconstruction sequence
#
...
def add_parser_args(parser):
    """Register all common CLI arguments onto *parser* (k4FWCore parseArgs parser)."""
    parser.add_argument("--dataFolder", type=str, help="Path to calibration data folder", default="./")
...
#   TopAlg           - the mutable list of top-level algorithms (modified in-place)
#   outputSaveClusters - the mutable list of cluster collection names to save (modified in-place)
#   cfg              - a SimpleNamespace (or any object) carrying detector/run parameters:
#                        cfg.IDs                       : dict mapping subdetector key -> system ID
#                        cfg.ecalBarrelLayers          : int
#                        cfg.ecalEndcapLayers          : int
#                        cfg.hcalBarrelLayers          : int
#                        cfg.ecalBarrelThetaWeights    : list[float]
#                        cfg.ecalBarrelUpstreamParameters   : list
#                        cfg.ecalBarrelDownstreamParameters : list
#                        cfg.dataFolder                : str
#                        cfg.doCreateClusterCellCollection : bool
#                        cfg.logEWeightInPhotonID      : bool
...
def setupSWClusters(inputCells,
    ...
    # Clustering parameters - phi-theta window sizes
    ...
    clusterAlg = CreateCaloClustersSlidingWindowFCCee("Create" + outputClusters,
                                                      ...
                                                      energyThreshold=clusteringThreshold,
                                                      energySharingCorrection=False,
                                                      createClusterCellCollection=cfg.doCreateClusterCellCollection,
                                                      OutputLevel=INFO
                                                      )
    ...
    if applyUpDownstreamCorrections:
        ...
        correctClusterAlg = CorrectCaloClusters("Correct" + outputClusters,
                                                ...
                                                systemIDs=[cfg.IDs["ECAL_Barrel"]],
                                                numLayers=[cfg.ecalBarrelLayers],
                                                firstLayerIDs=[0],
                                                lastLayerIDs=[cfg.ecalBarrelLayers - 1],
                                                readoutNames=[inputReadouts["ECAL_Barrel"]],
                                                upstreamParameters=cfg.ecalBarrelUpstreamParameters,
                                                upstreamFormulas=[['[0]+[1]/(x-[2])', '[0]+[1]/(x-[2])']],
                                                downstreamParameters=cfg.ecalBarrelDownstreamParameters,
        ...
        if outputClusters.startswith(("EMBCaloClusters", "EMECCaloClusters", "CaloClusters")):
            ...
            if outputClusters.startswith("EMBCaloClusters"):
                augmentClusterAlg = AugmentClustersFCCee("Augment" + outputClusters,
                                                         ...
                                                         thetaRecalcWeights=[cfg.ecalBarrelThetaWeights],
                                                         ...
                                                         do_widthTheta_logE_weights=cfg.logEWeightInPhotonID,
                                                         OutputLevel=INFO
                                                         )
                ...
                augmentClusterAlg = AugmentClustersFCCee("Augment" + outputClusters,
                                                         inClusters=clusterAlg.clusters.Path,
                                                         outClusters="Augmented" + clusterAlg.clusters.Path,
                                                         systemIDs=[cfg.IDs["ECAL_Endcap"]],
                                                         systemNames=["EMEC"],
                                                         numLayers=[cfg.ecalEndcapLayers],
                                                         readoutNames=[inputReadouts["ECAL_Endcap"]],
                                                         layerFieldNames=["layer"],
                                                         thetaRecalcWeights=[[-1]*cfg.ecalEndcapLayers],
                ...
                augmentClusterAlg = AugmentClustersFCCee("Augment" + outputClusters,
                                                         ...
                                                         #numLayers=[cfg.ecalBarrelLayers, cfg.ecalEndcapLayers, cfg.hcalBarrelLayers, cfg.hcalEndcapLayers],
                                                         ...
                                                         #thetaRecalcWeights=[cfg.ecalBarrelThetaWeights, [-1]*cfg.ecalEndcapLayers, [-1]*cfg.hcalBarrelLayers, [-1]*cfg.hcalEndcapLayers],
                                                         systemIDs=[cfg.IDs["ECAL_Barrel"],cfg.IDs["HCAL_Barrel"]],
                                                         systemNames=["EMB", "HCALB"],
                                                         numLayers=[cfg.ecalBarrelLayers, cfg.hcalBarrelLayers],
                                                         ...
                                                         thetaRecalcWeights=[cfg.ecalBarrelThetaWeights, [-1]*cfg.hcalBarrelLayers],
                                                         do_photon_shapeVar=False,
                                                         do_widthTheta_logE_weights=cfg.logEWeightInPhotonID,
                                                         OutputLevel=INFO
                                                         )
    ...
    if runTRAPPISTTool:
        if not addPi0RecoTool:
            ...
            TRAPPISTInferenceAlg = TRAPPISTPi0PhotonInference(
                ...
                ONNXModelPath=cfg.dataFolder + "TRAPPIST_SWclustering.onnx",
                OutputLevel=INFO,
            )
    ...
    if applyMVAClusterEnergyCalibration:
        ...
        calibrateClustersAlg = CalibrateCaloClusters("Calibrate" + outputClusters,
                                                     ...
                                                     readoutNames=[inputReadouts["ECAL_Barrel"]],
                                                     layerFieldNames=["layer"],
                                                     calibrationFile=cfg.dataFolder + "lgbm_calibration-CaloClusters.onnx",
                                                     OutputLevel=INFO
                                                     )
    ...
    if runPhotonIDTool:
        if not addShapeParameters:
            ...
            photonIDAlg = PhotonIDTool("PhotonID" + outputClusters,
                                       ...
                                       mvaModelFile=cfg.dataFolder + "bdt-photonid-weights-EMBCaloClusters.onnx",
                                       mvaInputsFile=cfg.dataFolder + "bdt-photonid-settings-EMBCaloClusters.json",
                                       OutputLevel=INFO
                                       )
            TopAlg += [photonIDAlg]
...
def setupTopoClusters(inputCells,
    ...
    if runTRAPPISTTool:
        if not addPi0RecoTool:
            ...
            TRAPPISTInferenceAlg = TRAPPISTPi0PhotonInference(
                ...
                inClusters=[inClusters],
                outClusters=[inClusters + "WithScore"],
                ONNXModelPath=cfg.dataFolder + "TRAPPIST_topoclustering.onnx",
                OutputLevel=INFO,
            )
    ...
    if applyMVAClusterEnergyCalibration:
        ...
        calibrateClustersAlg = CalibrateCaloClusters("Calibrate" + outputClusters,
                                                     ...
                                                     readoutNames=[inputReadouts["ECAL_Barrel"]],
                                                     layerFieldNames=["layer"],
                                                     calibrationFile=cfg.dataFolder + "lgbm_calibration-CaloTopoClusters.onnx",
                                                     OutputLevel=INFO
                                                     )
        TopAlg += [calibrateClustersAlg]
    ...
    if runPhotonIDTool:
        if not addShapeParameters:
            ...
            photonIDAlg = PhotonIDTool("PhotonID" + outputClusters,
                                       inClusters=inClusters,
                                       outClusters="PhotonID" + inClusters,
                                       mvaModelFile=cfg.dataFolder + "bdt-photonid-weights-EMBCaloTopoClusters.onnx",
                                       mvaInputsFile=cfg.dataFolder + "bdt-photonid-settings-EMBCaloTopoClusters.json",
                                       OutputLevel=INFO)
            TopAlg += [photonIDAlg]


# ---------------------------------------------------------------------------
# Main digitization/reconstruction function
# ---------------------------------------------------------------------------

def run_digi_reco(path_to_detector, detectors_to_use, wire_tracker):
    """Run the full ALLEGRO digitization and reconstruction sequence.

    Parameters
    ----------
    path_to_detector : str
        Absolute path to the detector compact directory (ending with '/').
    detectors_to_use : list[str]
        List of XML filenames (relative to path_to_detector) that describe the
    ...
    # override dataFolder default to the value set above
    parser.set_defaults(dataFolder=dataFolderDef)
    ...
    dropWireHits = wire_tracker.drop_hits
    dropSiWrHits = False
    dropMuonHits = False
    ...
    for constant in root.find('define').findall('constant'):
        if (
            ...
            or constant.get('name') == wire_tracker.det_id_key
    ...
    if addTruthTracks:
        ...
        dNdxFromTracks = TrackdNdxDelphesBased("dNdxFromTracks",
                                               ...
                                               OutputCollection=[wire_tracker.dNdx_output_collection],
                                               ZmaxParameterName=wire_tracker.dNdx_Zmax_param,
                                               ZminParameterName=wire_tracker.dNdx_Zmin_param,
                                               RminParameterName=wire_tracker.dNdx_Rmin_param,
                                               RmaxParameterName=wire_tracker.dNdx_Rmax_param,
                                               FillFactor=wire_tracker.dNdx_fill_factor,
                                               OutputLevel=ERROR)
    ...
    if runTrkHitDigitization:
        ...
        wire_digi_kwargs = dict(
            InputSimHitCollection=[wire_tracker.hit_collection],
            OutputDigihitCollection=[wire_tracker.digi_collection],
            OutputLinkCollection=[wire_tracker.sim_digi_links],
            DCH_name=wire_tracker.dchi_name,
    ...
    if runTrkFinder:
        ...
        trackFinder = GGTFTrackFinder(
            ...
            InputWireHitCollections=[wire_tracker.digi_collection],
    ...
    if runTrkValidation:
        ...
        perfect_finder = PerfectTrackFinder(
            ...
            InputWireHitCollections=[wire_tracker.sim_digi_links],
            OutputPerfectTracks=["PerfectPrefitTracks"],
            OutputLevel=INFO
        )
        perf_fitter_kwargs = dict(
        ...
        trackValidation = TrackingValidation(
            ...
            HitSimLinks=["VTXBSimDigiLinks","VTXDSimDigiLinks","SiWrBSimDigiLinks","SiWrDSimDigiLinks",wire_tracker.sim_digi_links],
    ...
    cfg = SimpleNamespace(
        IDs=IDs,
        ecalBarrelLayers=ecalBarrelLayers,
        ecalEndcapLayers=ecalEndcapLayers,
        hcalBarrelLayers=hcalBarrelLayers,
        ecalBarrelThetaWeights=ecalBarrelThetaWeights,
        ecalBarrelUpstreamParameters=ecalBarrelUpstreamParameters,
        ecalBarrelDownstreamParameters=ecalBarrelDownstreamParameters,
        dataFolder=dataFolder,
        doCreateClusterCellCollection=doCreateClusterCellCollection,
        logEWeightInPhotonID=logEWeightInPhotonID,
    )
    ...
    if doSWClustering:
        ...
        if (runMuon):
            ...
            setupSWClusters(MuonCaloClusterInputs,
                            ...
                            False,
                            False,
                            TopAlg, outputSaveClusters, cfg, "MuonSize")
    ...
    if dropWireHits:
        io_svc.outputCommands.append("drop " + wire_tracker.drop_hits_command)
    if dropSiWrHits:
